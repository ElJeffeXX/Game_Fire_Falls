...
function applyUpgrade(card) {
  ...
  if (b.delta !== undefined) {
    if (b.key === "health") {
      ...
      const newVal = (player[b.key] || 0) + b.delta;
      player[b.key] = Math.max(1, newVal);
    }
  } else if (b.mult !== undefined) {
    if (b.key === "maxBeamRange") {
      maxBeamRange = parseFloat((maxBeamRange * b.mult).toFixed(1));
    } else {
      player[b.key] = parseFloat(((player[b.key] || 1) * b.mult).toFixed(3));
    }
  }
  ...
  if (card.debuff) {
    ...
    if (d.delta !== undefined) {
      ...
      if (d.key === "maxBeamRange") {
        maxBeamRange = parseFloat((maxBeamRange * d.mult).toFixed(1));
      } else {
        player[d.key] = parseFloat(((player[d.key] || 1) * d.mult).toFixed(3));
      }
    }
  }
}
...
function initUpgradeSystem() {
  ...
  s.textContent = `
    ...
    #upgrade-stats {
      ...
      gap: 10px;
      padding: 18px 8px 0;
      border-top: 1px solid rgba(245, 230, 200, 0.18);
      margin-top: 8px;
    }
    .upgrade-stat-entry {
      display: flex;
    ...
    .upgrade-stat-entry .stat-label {
      opacity: 0.85;
    }
    .upgrade-stat-entry .stat-value {
      font-weight: 700;
      color: #ffffff;
    }
    #upgrade-panel,
    ...
    .upg-card.card-fade-out {
      opacity: 0;
      transform: translateY(-20px) scale(0.85);
    }
    .upg-card.card-selected {
      animation: card-selected 0.5s ease forwards;
    }
    .upg-card.card-skip-away {
      animation: card-skip-away 0.45s ease forwards;
    }
    ...
    @keyframes card-selected {
      ...
      50% {
        opacity: 1;
        transform: translateX(0) scale(1.25) rotate(180deg);
      }
      100% {
        opacity: 0;
    ...
    #upgrade-reroll:hover:not(:disabled) {
      color: #f5e6c8;
      border-color: #b89a6a;
      background: rgba(100, 70, 20, 0.1);
    }
...
function handleReroll() {
  ...
  cards.forEach((upgrade, index) => {
    ...
    cardEl.addEventListener("click", () => {
      animateUpgradeChoice(cardEl, upgrade);
    });
...
function getRoundMultiplier(round) {
  const milestones = Math.floor(round / 30);
  return Math.pow(1.4, milestones);
}
...
function getRoundConfig(round) {
  ...
  const spawnInterval = Math.max(150 - r * 80, 600);

  return { total, maxOnScreen, minHp, maxHp, minSpeed, maxSpeed, spawnInterval };
}
...
function startRound(round) {
  ...
  spawnIntervalId = setInterval(() => {
    if (
      ...
      enemies.length >= roundConfig.maxOnScreen
    ) return;
...
function spawnRoundEnemy() {
  ...
  const enemyType = getEnemyTypeForRound(currentRound);
...
splashImg.src = "Sprites/splash/splash.png";

let dripSheet = new Image();
dripSheet.src = "Sprites/drip/drip.png";

let redFireSheet = new Image();
redFireSheet.src = "Sprites/fire/fireRed.png";

let greenFireSheet = new Image();
greenFireSheet.src = "Sprites/fire/fireGreen.png";
let purpleFireSheet = new Image();
purpleFireSheet.src = "Sprites/fire/firePurple.png";
let blueFireSheet = new Image();
blueFireSheet.src = "Sprites/fire/fireBlue.png";
let greyFireSheet = new Image();
greyFireSheet.src = "Sprites/fire/fireGrey.png";

let flowerHpSheet = new Image();
flowerHpSheet.src = "Sprites/Health/FlowerHp.png";
let worldHpSheet = new Image();
worldHpSheet.src = "Sprites/Health/WorldHp.png";
...
function drawGameOver() {
  ...
  if (msgEl) {
    msgEl.textContent = world.health <= 0 ? "The world was destroyed." : "You were defeated.";
  }
...
function drawRoundAnnouncement() {
  ...
  if (roundAnnouncementTimer > 150) {
    roundAnnouncementAlpha = 1.0;
  } else {
    roundAnnouncementAlpha = roundAnnouncementTimer / 150;
  }
...
function drawRoundHUD() {
  const padding = 14;
  const text1 = `Round ${currentRound}`;
  const text2 = `${enemiesDefeated} / ${totalEnemiesThisRound} defeated`;
  ...
  gameCtx.fillText(text1, gameCanvas.width - padding, padding + 20);
  gameCtx.font = "16px sans-serif";
  gameCtx.fillText(text2, gameCanvas.width - padding, padding + 42);
...
function drawHealthBars() {
  ...
  if (flowerHpSheet.complete && flowerHpSheet.naturalHeight !== 0) {
    let frameW = flowerHpSheet.width / totalFrames;
    let frameH = flowerHpSheet.height;
    let playerHealth = Math.max(0, Math.min(5, player.health));
    let frame = 5 - playerHealth;
    ...
    gameCtx.drawImage(
      ...
      baseX, y,
      frameW * hpBarScale, frameH * hpBarScale
    );
  }

  if (worldHpSheet.complete && worldHpSheet.naturalHeight !== 0) {
    let frameW = worldHpSheet.width / totalFrames;
    let frameH = worldHpSheet.height;
    let worldHealth = Math.max(0, Math.min(5, world.health));
    let frame = 5 - worldHealth;
...
function drawGame() {
  ...
  for (let i = drips.length - 1; i >= 0; i--) {
    if (dripSheet.complete && dripSheet.naturalHeight !== 0) {
      ...
      gameCtx.drawImage(
        dripSheet,
        drips[i].frame * frameW, 0, frameW, frameW,
  ...
  if (sheet) {
    ...
    gameCtx.drawImage(
      sheet, frame * frameWidth, 0, frameWidth, frameHeight,
  ...
  if (orbSheet && orbSheet.complete && orbSheet.naturalHeight !== 0) {
    ...
    gameCtx.drawImage(
      orbSheet, frame * orbFrameWidth, 0, orbFrameWidth, orbFrameHeight,
  ...
  for (let i = 0; i < enemies.length; i++) {
    ...
    let numFrames = 10;
    let frame = enemy.animationFrame % numFrames;
    let fireSheet = null;
  ...
  if (beamActive && mouse.down) {
    let maxBeamLength = maxBeamRange;
    let { x: orbX, y: orbY } = getOrbPosition();
    let targetX = Math.max(0, Math.min(gameCanvas.width, mouse.x));
    let targetY = Math.max(0, Math.min(gameCanvas.height, mouse.y));
    let dx = targetX - orbX;
    let dy = targetY - orbY;
    let distance = Math.hypot(dx, dy);
    let dirX = dx / Math.max(distance, 0.0001);
    let dirY = dy / Math.max(distance, 0.0001);
    let beamLength = Math.min(distance, maxBeamLength);
    ...
    for (let i = enemies.length - 1; i >= 0; i--) {
      ...
      let dist = pointToSegmentDistance(enemyX, enemyY, startX, startY, endX, endY);

      if (dist < beamThickness + 5) {
      ...
      let endDistX = endX - enemyX;
      let endDistY = endY - enemyY;
      let endDist = Math.hypot(endDistX, endDistY);
      if (endDist < 10) {
        if (enemy.hitCooldown2 <= 0) {
    ...
    gameCtx.fillRect(0, -beamWidth / 2, length, beamWidth);
    gameCtx.fillStyle = grad;
...
// Game over checked FIRST before round completion
  checkGameOver();
  if (gameOver) {
    drawGameOver();
  } else {
...
function playerMovement() {
  ...
  if (player.dashing) {
    ...
    if (player.dashDirection === "w") player.y -= dashMoveAmount;
    else if (player.dashDirection === "s") player.y += dashMoveAmount;
    else if (player.dashDirection === "a") player.x -= dashMoveAmount;
    else if (player.dashDirection === "d") player.x += dashMoveAmount;
    if (player.dashDirection === "arrowup") player.y -= dashMoveAmount;
    else if (player.dashDirection === "arrowdown") player.y += dashMoveAmount;
    else if (player.dashDirection === "arrowleft") player.x -= dashMoveAmount;
    else if (player.dashDirection === "arrowright") player.x += dashMoveAmount;
...
function movementAnimation() {
  ...
  function animateStep() {
    ...
    for (let i = 0; i < enemies.length; i++) {
      ...
      enemy.animationFrame = enemy.animationCounter % 10;
    }
    animationCounter++;
...
function enemyMovement() {
  if (!gameOver) {
    for (let i = 0; i < enemies.length; i++) {
      ...
      if (enemy.hitCooldown2 > 0) enemy.hitCooldown2--;
    }
    col("fireR", "floor");
  }
...
function col(entity, colType) {
  if (entity == "player") {
    ...
    if (colType == "enemy") {
      for (let i = 0; i < enemies.length; i++) {
        const enemy = enemies[i];
        let px = player.x + player.width * 0.2;
        let py = player.y + player.height * 0.2;
        let pw = player.width * 0.6;
